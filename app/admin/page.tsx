...
export default async function AdminPage() {
  ...
  // 4. Fetch users (for Admin role management and Ban/Suspend/Unban)
  const { data: usersRaw } = await supabase
    .from('users')
    .select('id, email, full_name, avatar_url, role, status, ban_reason, banned_at, suspended_until, academic_year, major')
    .order('created_at', { ascending: false });
